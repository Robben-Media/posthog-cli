...
const (
	apiKeyKey          = "api_key"
	keyringPasswordEnv = "POSTHOG_CLI_KEYRING_PASS" //nolint:gosec // env var name, not a credential
	keyringBackendEnv  = "POSTHOG_CLI_KEYRING_BACKEND"
...
func ResolveKeyringBackendInfo() (KeyringBackendInfo, error) {
	if v := normalizeKeyringBackend(os.Getenv(keyringBackendEnv)); v != "" {
		return KeyringBackendInfo{Value: v, Source: "env"}, nil
	}
...
func openKeyringWithTimeout(cfg keyring.Config, timeout time.Duration) (keyring.Keyring, error) {
	...
	case <-time.After(timeout):
		return nil, fmt.Errorf("%w after %v (D-Bus SecretService may be unresponsive); "+
			"set POSTHOG_CLI_KEYRING_BACKEND=file and POSTHOG_CLI_KEYRING_PASS=<password> to use encrypted file storage instead",
